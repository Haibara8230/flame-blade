/* 第二部无头通关测试：直接从 part2_open 起跑，验证第七章到终幕能不能走完。
   第一部由 playthrough.cjs 覆盖。 */
...
const START = process.argv[3] || 'part2_open';     // 起始场景
const LV = Number(process.argv[4] || 20);          // 队伍等级
const ROUNDS = Number(process.argv[5] || 60);      // 最多跑几轮
...
(async () => {
  ...
  await shot('p2-' + START);
  console.log('start:', await evalx(`
    window.__fast = true;
    document.getElementById('btn-new').click();
    window.__partyJoin('lei', ${LV}); window.__partyJoin('cang', ${LV}); window.__partyJoin('ryze', ${LV});
    // 第二部的入口条件：第一部拿到 TRUE / SECRET，并且把三地支援都做齐了
    Object.assign(window.__G.flags, {
      trustRyze: true, promised: true, sealKnowledge: true, sharedSeal: true,
      forestAid: true, harborAid: true, northAid: true, allAid: true,
      part1Ending: 'secret', partShown_secret: true,
    });
    window.__levelTo(${LV});
    window.__setScene('${START}');
    'ok'`));
  let last = '';
  for (let round = 0; round < ROUNDS; round++) {
    ...
    if (typeof st === 'string' && st.includes('"mode":"ending"')) { await shot('p2-end-' + START); break; }
    if (st === last && round > 0) { console.log('no progress, stopping'); break; }
    last = st;
    if (round % 8 === 7) await shot('p2-' + START + '-' + round);
  }
