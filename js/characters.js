...
/* ---------------- 道具 ----------------
   只放原文明确给出属性的。⚠ 此前这里是空的。 */
export const ITEMS = {
  /* 原文第14章：「小恢复药水：瞬间恢复生命100，冷却时间5秒，
     效果不可与同类恢复道具叠加。」
     ⚠ 冷却与「不可叠加」两条引擎还没实现，这里只落了回复量。 */
  potion_s: {
    id: 'potion_s', name: '小恢复药水', heal: 100, price: 2,
    desc: '瞬间恢复生命100，冷却时间5秒，效果不可与同类恢复道具叠加。',
  },
  /* 原文第14章：「苹果：一种普通的水果，食用后生命恢复50，饥饿度-10。」 */
  apple: {
    id: 'apple', name: '苹果', heal: 50, price: 0,
    desc: '一种普通的水果，食用后生命恢复50，饥饿度-10。',
  },
  /* 原文第14章：「馒头：一种普通的面食，食用后后生命恢复10，饥饿度-30。」 */
  bun: {
    id: 'bun', name: '馒头', heal: 10, price: 0,
    desc: '一种普通的面食，食用后生命恢复10，饥饿度-30。',
  },
};

/* ⚠ 饥饿度：原文第7章给了完整规则——
     「饥饿度：0/110（承受上限=100+力量属性，达到上限时每秒自动掉落1%生命）」
     第23章补充：城镇中每5分钟+1，野外战斗状态每3分钟+1。
   引擎还没有这套系统，苹果与馒头的「饥饿度-N」目前不生效。
   等做饥饿度的时候，数值按上面两条照抄，不要重新拍。 */
